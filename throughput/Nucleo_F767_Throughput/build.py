#!/usr/bin/env python3
"""Build and flash helper for Nucleo_F767_Throughput.

Usage:
    python3 build.py                     # build the default transport
    python3 build.py --transport rtt        # build another transport variant
...
switching never hits a stale CMake cache):
    rambuf   RAM ring drained through the J-Link (default)
    rtt      SEGGER RTT (vendored SEGGER_RTT sources)
...
PROJECT = "Nucleo_F767_Throughput"
ELF_NAME = "Nucleo_F767_Throughput"
PROBE = "jlink"                      # "stlink" or "jlink"
TARGET_DEVICE = "STM32F767ZI"      # device name for J-Link flashing
TRANSPORTS = {"rambuf": "RAM_BUFFER", "rtt": "JLINK_RTT"}              # cli name -> VA_TRANSPORT value (None = project default)
