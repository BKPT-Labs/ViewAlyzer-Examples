...
1. Create a Zephyr application that uses ViewAlyzer as an external module, like this `STM32-Zephyr-VA-Demo` project.
...
- app source lives here in `STM32-Zephyr-VA-Demo`
...
In this example project, the actual CMakeLists.txt goes one step further: it
resolves `VIEWALYZER_MODULE_PATH` from (in order) a `-D` option on the
command line, the environment, or a `ViewAlyzer` checkout sitting next to
this examples repo — and fails with a clear message if none of those exist.
You can copy that block verbatim, or just hardcode the path as above.
...
Zephyr plus J-Link may need a short delay before `VA_Init()` so the debugger can attach in time to catch the initial sync marker.
...
When you test this tutorial, follow this order:
