...
// Created BEFORE VA_Init() to exercise lazy registration on first switch-in.
// Also the event source for the GPIO channel and manual heap gauge.
static void EarlyTask(void *argument)
{
    (void)argument;
    for (;;)
    {
        VA_LogGPIO(1, (HAL_GetTick() / 100u) & 1u);
        VA_LogHeap(1, 1024u + (HAL_GetTick() % 512u));
...
int main(void)
{
 ...
 // SWO_Init(170000000u, 2000000u, 1);
  ...
  // Deliberately before VA_Init: this task's create-hook registration gets
  // wiped by VA_Init, so it must reappear via lazy switch-in registration.
  xTaskCreate(EarlyTask, "EarlyTask", 256, NULL, tskIDLE_PRIORITY + 1, NULL);

  VA_Init(SystemCoreClock);
                                                        
  // Every id logged by app_freertos.c must be registered here, or the viewer
  // falls back to "USER_<id>". Names are capped at VA_MAX_TASK_NAME_LEN-1 (15)
  // chars by the recorder -- anything longer is silently truncated.
  ...
  // Normal (non-inverting) mutex task traces
  VA_RegisterUserTrace(54, "Norm Low Value", VA_USER_TYPE_GRAPH);  // NormalLowPrio: shared value after update
  VA_RegisterUserTrace(56, "Norm High Value", VA_USER_TYPE_GRAPH); // NormalHighPrio: shared value after update

  // Float traces
  VA_RegisterUserTrace(60, "Sine Float", VA_USER_TYPE_GRAPH);     // Task02: sinf() sine wave

  // GPIO channel + manual heap gauge (logged by EarlyTask). Their name maps
  // must also reach a host that attaches mid-run via the periodic bundle.
  VA_RegisterGPIO(1, "TestPin");
