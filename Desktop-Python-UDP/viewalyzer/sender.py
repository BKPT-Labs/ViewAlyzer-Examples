...
from viewalyzer.protocol import (
    TraceType, SeqStamper,
    FLAG_GROUP_BUILD, BUILD_HOST_TS,
    build_sync, build_info_clk, build_config_flags, build_session_start,
...
class ViewAlyzerSender:
    ...
    Every packet carries the rolling sequence byte (part of the wire
    format), so the receiver can *prove* whether any packets were lost
    in transit (see ``protocol.SeqStamper``).
    """
    ...
    def send_framed(self, raw_packet: bytes) -> None:
        """Stamp, COBS-encode, and send one raw VA packet via UDP."""
        raw_packet = self._stamper.stamp(raw_packet)
        self._sock.sendto(cobs_encode(raw_packet), self._dest)

    def send_marker(self) -> None:
        """Send the sync marker. Markers are not packets: never stamped,
        they consume no sequence number."""
        self._sock.sendto(cobs_encode(build_sync()), self._dest)

    def send_batch(self, packets: list[bytes], mtu: int = 1400) -> None:
        """Stamp + COBS-encode multiple packets and send in MTU-sized
        UDP datagrams."""
        buf = bytearray()
        for pkt in packets:
            pkt = self._stamper.stamp(pkt)
            buf += cobs_encode(pkt)
        offset = 0
    ...
    def send_sync_and_clock(self) -> None:
        """Send the sync marker, session start, CLK info packet, sequence
        checkpoint, and (if enabled) the HOST_TS flag - the same session
        prelude as the firmware's VA_Init (SES:START must precede the
        checkpoint: the receiver resets its counter epoch on SES)."""
        self.send_marker()
        self.send_framed(build_session_start())
        self.send_checkpoint()
        self.send_framed(build_info_clk(self._cpu_freq))
        if self._host_timestamps:
            self.send_framed(build_config_flags(FLAG_GROUP_BUILD, BUILD_HOST_TS))

    def send_checkpoint(self, timestamp: int = 0) -> None:
        """Send an absolute sequence checkpoint. Call periodically
        (e.g. with each re-sync) so the receiver can compute exact loss
        totals across bursts longer than the 8-bit rolling counter."""
        pkt = self._stamper.checkpoint(self._ts(timestamp))
        self._sock.sendto(cobs_encode(pkt), self._dest)
