/*
 * ViewAlyzer on the ST NUCLEO-C031C6 (STM32C031C6, Cortex-M0+).
 *
 * The 16-BIT timer-source reference: the C031 has no DWT, no ITM, and no
 * 32-bit timer, so timestamps come from 16-bit TIM3 through the recorder's
 * CUSTOM_TIMER source with VA_TIMER_BITS=16, and the transport is the RAM
 * buffer drained by the on-board ST-LINK.
 *
 * Kept deliberately small (12 KB RAM / 32 KB flash): a heartbeat in main
...
#include <soc.h>                     /* TIM3 / RCC register definitions */
...
/* ---------------- Timestamp source: TIM3 as a 100 kHz free-runner -------
 * TIM3 is 16-bit, so the wrap budget rules from ViewAlyzerConfig.h apply:
 ...
 *
 * The timer kernel clock equals the CPU clock (48 MHz) because this board
 * config runs AHB and APB undivided (Zephyr's nucleo_c031c6 devicetree);
 ...
 * project claims TIM3 (no PWM/counter driver is enabled).
 */
#define TICK_HZ           100000u
#define TIM3_CLOCK_HZ     ((uint32_t)CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC)
...
static void tick_timer_init(void)
{
    ...
    TIM3->PSC = (uint16_t)((TIM3_CLOCK_HZ / TICK_HZ) - 1u);
    TIM3->ARR = 0xFFFFu;             /* full 16-bit span */
    TIM3->EGR = TIM_EGR_UG;          /* latch PSC/ARR now, not at wrap */
    TIM3->CR1 = TIM_CR1_CEN;
}

static uint32_t tick_timer_read(void)
{
    return TIM3->CNT;                /* low 16 bits used (VA_TIMER_BITS=16) */
}
...
K_THREAD_DEFINE(worker_tid, 768, worker_thread, NULL, NULL, NULL,
