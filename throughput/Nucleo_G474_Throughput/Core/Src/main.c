/**
 * Nucleo-G474RE — ViewAlyzer transport throughput benchmark (bare-metal).
 *
 * Cortex-M4F @ 170 MHz sibling of the Nucleo_H723_Throughput project: the
 * same paced saturation loop and _VA_TP offered/dropped accounting
 * (-DVA_TP_TEST=1), aimed at the board's on-board ST-LINK:
 *
 ...
 *
 * Bring-up is a HAL clock config (HSI16 -> PLL -> 170 MHz, boost regulator);
 * everything else runs on plain CMSIS registers.
 */
#include "main.h"
...
/* Green LD2 on the Nucleo-G474RE (PA5) — a visible heartbeat so a flashed
...
int main (void)
{
    ...
    for (;;)
    {
        ...
        {
            last_log_ms = now_ms;
            VA_LogString (TRACE_LOG, "Nucleo-G474 throughput benchmark running");
        }
