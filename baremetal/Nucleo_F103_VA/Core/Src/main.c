/**
 * Nucleo-F103RB — ViewAlyzer bare-metal integration example (Cortex-M3 @ 72 MHz).
 *
 * A normal-usage reference (the benchmark sibling lives in
 * throughput/Nucleo_F103_Throughput): user value traces, event spans, and
...
/* Green LD2 on the Nucleo-F103RB (PA5) */
...
int main (void)
{
    ...
    for (;;)
    {
        ...
        {
            led_state = ! led_state;
            GPIOA->ODR ^= GPIO_ODR_ODR5;               /* green LD2 */
            VA_LogToggle (TRACE_LED, led_state);
        }

        if ((step & 0x7FFFFu) == 0u)
            VA_LogString (TRACE_LOG, "Nucleo-F103 bare-metal demo alive");
