# Nucleo-H503RB - Transport Throughput Benchmark

Bare-metal (no RTOS) benchmark that measures **how many trace bytes per second
a ViewAlyzer transport can actually sustain** from a Nucleo-H503RB
(STM32H503RB, Cortex-M33 @ 240 MHz) to the host, over the board's on-board
...
```sh
# RAM buffer
ViewAlyzer --headless --config nucleo_h503_rambuf.vacf \
           --output tp_rambuf.vadb --duration 20 --verbose
# SWO / ITM
ViewAlyzer --headless --config nucleo_h503_swo.vacf \
