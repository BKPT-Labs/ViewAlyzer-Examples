# Nucleo-G474RE - ViewAlyzer Bare-metal Example

Bare-metal (no RTOS) integration reference for the Nucleo-G474RE
(STM32G474RE, Cortex-M4F @ 170 MHz): user value traces, event spans, and ISR
...
- **RAM buffer** (default) - the recorder owns a RAM ring the ViewAlyzer app
  drains through the on-board ST-LINK V3 with non-intrusive memory reads. No
...
```sh
ViewAlyzer --headless --config nucleo_g474_rambuf.vacf \
           --output demo.vadb --duration 10 --verbose
```

For the transport bandwidth ceilings of this board, see the benchmark sibling
[`throughput/Nucleo_G474_Throughput`](../../throughput/Nucleo_G474_Throughput/).
